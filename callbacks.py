...
def get_compare_page(AY: pd.DataFrame, current_district, compare_range_slider, compare_objective, gpa_type):
    ...
    if not compare_objective:
        compare_objective = 'GPA'
    ...
    match compare_objective:
        case 'GPA':
            if len(district_AY[district_AY['Grade Level'].isin(['9', '10', '11', '12'])]) == 0:
                objective_compare = html.Div([html.H4('No GPAs for the current district and filters', id='compare-no-data-message')], className='graph')
            else:
                objective_compare = charts.get_gpa_compare(AY, district_AY, gpa_type)
        case 'FAFSA':
            if len(district_AY[district_AY['Grade Level'].isin(['12'])]) == 0:
                objective_compare = html.Div([html.H4('No FAFSA data for the current district and filters', id='compare-no-data-message')], className='graph')
            else:
                objective_compare = charts.get_fafsa_compare(AY[AY['Grade Level'] == '12'], district_AY[district_AY['Grade Level'] == '12'])
        case 'Graduation':
            if len(district_AY[district_AY['Grade Level'].isin(['12'])]) == 0:
                objective_compare = html.Div([html.H4('No graduation data for the current district and filters', id='compare-no-data-message')], className='graph')
            else:
                objective_compare = charts.get_graduation_compare(AY[AY['Grade Level'] == '12'], district_AY[district_AY['Grade Level'] == '12'])
        case 'Post Secondary Enrollment':
            if len(district_AY[district_AY['Grade Level'].isin(['12'])]) == 0:
                objective_compare = html.Div([html.H4('No PSE data for the current district and filters', id='compare-no-data-message')], className='graph')
            else:
                objective_compare = charts.get_pse_compare(AY[AY['Grade Level'] == '12'], district_AY[district_AY['Grade Level'] == '12'])

    return components.get_compare_layout(district_list, current_district, range_low, range_high, compare_objective, gpa_type, district_services_time_split, program_services_time_split, service_participation_compare, objective_compare)
...
def register_callbacks(app, AY_df: pd.DataFrame, agg_services_df: pd.DataFrame, duration_by_student_month_type: pd.DataFrame, college_visits: pd.DataFrame):
    ...
    def update_page(selected_year, demographics_btn, services_btn, services_yty_btn, objectives_btn, objectives_yty_btn, combare_btn, district_click, gender_click, ethnicity_click, grade_click, race_click, remove_clicks, services_threshold, services_type_filter, yty_threshold, yty_type_filter, current_district, compare_range_slider, compare_objective, compare_gpa_type, gpa_type, gpa_range, gpa_benchmark, sankey_l1_option, sankey_l2_option, sankey_l3_option, sankey_l4_option, sankey_click, yty_gpa_radio, gpa_yty_benchmark_input, gpa_yty_increase_input, gpa_yty_benchmark_year, fafsa_yty_benchmark_input, fafsa_yty_increase_input, fafsa_yty_benchmark_year, graduation_yty_benchmark_input, graduation_yty_increase_input, graduation_yty_benchmark_year, pse_yty_benchmark_input, pse_yty_increase_input, pse_yty_benchmark_year, active_filters, current_page):
        ...
        match page:
            ...
            case 'compare':
                contents = get_compare_page(filtered_AY_with_year, current_district, compare_range_slider, compare_objective, compare_gpa_type)
                page_lable = 'Compare'
