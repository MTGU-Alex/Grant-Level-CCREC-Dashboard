...
mark_colors = ["#0F5C83", "#d75426", "#898E3E", "#EDD9B4" ,"#6B3F20" ,"#C4882A" ,"#2A3D4A" ,"#7f7f7f" ,"#7A8C5E" ,"#17becf"]
...
CCREC_black = '#231F20'
CCREC_dark_grey = '#6E7072'
CCREC_grey = '#818386'
CCREC_light_grey = '#A8AAAC'
...
def get_double_stacked_formatting(groups):
    if groups == 1:
        breaks = 10
        nudges = 35
        prespaces = 2
    ...
    return breaks, nudges, prespaces, standoff
...
def get_participation_ranges(AY: pd.DataFrame, participation_low, participation_high, group_lable): 
    ...
    return participation_range_by_grade.pivot(index='Grade Level', columns='Participation Range', values='Percent').reset_index().fillna(0)
...
def get_service_participation_compare(AY: pd.DataFrame, district_AY, participation_low, participation_high):
    ...
    district_participation = get_participation_ranges(district_AY, participation_low, participation_high, 'District')

    district_grades = district_participation['Grade Level'].to_list()
    program_participation = program_participation[program_participation['Grade Level'].isin(district_grades)]

    stack_cols = [f'Low: <= {participation_low}', f'Medium: > {participation_low}; <= {participation_high}', f'High: > {participation_high}', 'No Participation']

    color_map = {
        f'Low: <= {participation_low}': CCREC_orange, 
        f'Medium: > {participation_low}; <= {participation_high}': CCREC_blue, 
        f'High: > {participation_high}': CCREC_green, 
        'No Participation': CCREC_light_grey
    }

    fig = go.Figure()

    for count, df in enumerate([program_participation, district_participation]):
        for col in stack_cols:
            if col not in df:
                continue
            fig.add_trace(
    ...
    breaks, nudges, prespaces, standoff = get_double_stacked_formatting(len(district_grades))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            ticktext=[f'{'&nbsp;'*nudges}{' '*prespaces}{grade}th - district{'<br>'*breaks}{grade}th - program' for grade in district_grades],
            tickangle=45,
...
def get_avg_gpa_by_grade(df: pd.DataFrame, gpa_type, group_lable):
    gpas = df[['Grade Level', gpa_type]]
    gpas = gpas[~gpas['Grade Level'].isin(['7', '8', '13'])]
    gpas = gpas[(gpas[gpa_type] != 9.99) & (~gpas[gpa_type].isna()) & (gpas[gpa_type] != 0)]
    gpas_by_grade = gpas.groupby('Grade Level')[gpa_type].mean().to_frame('Average GPA').reset_index()
    gpas_by_grade['Average GPA'] = round(gpas_by_grade['Average GPA'], 2)
    gpas_by_grade['Group'] = group_lable
    return gpas_by_grade

def get_gpa_compare(AY: pd.DataFrame, district_AY: pd.DataFrame, gpa_type):
    program_gpas = get_avg_gpa_by_grade(AY, gpa_type, 'Program')
    district_gpas = get_avg_gpa_by_grade(district_AY, gpa_type, 'District')
    gpas = pd.concat([program_gpas, district_gpas])
    gpa_fig = px.bar(gpas,
        x='Grade Level',
        y='Average GPA',
        color='Group',
        barmode='group',
        title=f'Average GPA ({gpa_type})',
        text_auto=True
    )
    return gpa_fig

def get_general_percent(df: pd.DataFrame, col, group_lable):
    counts = df.groupby(col).size().to_frame('Student Count').reset_index()
    counts['Percent'] = round(counts['Student Count'].transform(lambda x: (x / x.sum()) * 100), 1)
    counts['Group'] = group_lable
    return counts

def get_fafsa_compare(AY: pd.DataFrame, district_AY: pd.DataFrame):
    fafsa_df = pd.concat([get_general_percent(AY, 'FAFSA status code', 'Program'), get_general_percent(district_AY, 'FAFSA status code', 'District')])
    fafsa_fig = px.bar(fafsa_df,
        x='Group',
        y='Percent',
        color='FAFSA status code',
        barmode='stack',
...
def get_graduation_compare(AY: pd.DataFrame, district_AY: pd.DataFrame):
    graduation_df = pd.concat([get_general_percent(AY, 'HS Grad Status code', 'Program'), get_general_percent(district_AY, 'HS Grad Status code', 'District')])
    graduation_fig = px.bar(graduation_df,
        x='Group',
        y='Percent',
        color='HS Grad Status code',
        barmode='stack',
    ...
    return graduation_fig

def get_pse_compare(AY: pd.DataFrame, district_AY: pd.DataFrame): 
    AY['Post Secondary Enrollment'] = np.where(((AY['First College Attended Name'].isna()) | (AY['First College Attended Name'].str.lower()=='not found')), 'Did Not Enroll', 'Enrolled')
    district_AY['Post Secondary Enrollment'] = np.where(((district_AY['First College Attended Name'].isna()) | (district_AY['First College Attended Name'].str.lower()=='not found')), 'Did Not Enroll', 'Enrolled')
    pse_df = pd.concat([get_general_percent(AY, 'Post Secondary Enrollment', 'Program'), get_general_percent(district_AY, 'Post Secondary Enrollment', 'District')])
    pse_fig = px.bar(pse_df,
        x='Group',
        y='Percent',
        color='Post Secondary Enrollment',
        barmode='stack',
