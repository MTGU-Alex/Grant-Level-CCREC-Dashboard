...
def get_compare_layout(district_list, current_district, range_low, range_high, objective_selection, gpa_type, district_services_time_split, program_services_time_split, service_participation_compare, objective_compare):
    if objective_selection == 'GPA':
        gpa_radio = dcc.RadioItems(['Cumulative GPA', 'Final Term GPA'], gpa_type, inline=True, id='compare-gpa-radio')
    else:
        gpa_radio = None

    if type(objective_compare) == html.Div:
        objectives_content = objective_compare
    else:
        objectives_content = dcc.Graph(figure=objective_compare, id='objective-compare', className='graph')
